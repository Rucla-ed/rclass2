...
if (!exists("els")) { # if object named "els" does not exist, load in stata dataset
  load("../../_data/els/els.RData")
}
...
if (!exists("els")) { # if object named "els" does not exist, read in stata dataset
  els <- read_dta(file="../../_data/els/els_02_12_byf3pststu_v1_0.dta") %>%
    # keep only subset of vars
    select(one_of(els_keepvars)) %>%
    # lower variable names
    rename_all(tolower)
...
- The variables of interest are high school math test score (`bynels2m`), earnings in 2011 (`f3ern2011`), and student gender (`f1sex`)
...
**Student Task**: Using the `els` dataset, create a scatterplot of the relationship between HS math score (`bynels2m`) on the x-axis and 2011 earnings (`f3ern2011`) on the y-axis, with the color of points determined by sex (`f1sex`)
...
**Student Task**: Using the `els` dataset, create a bar chart with the variable "ever attended postsecondary education" (`f2evratt`) as x-axis and number of students as y-axis
