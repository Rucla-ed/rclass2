...
2. Select `Inspect`  
3. Refresh the page
4. Under the `Network` tab, you can select any HTTP request on the left panel and view the HTTP headers on the right panel
...
FOR THE CLASS WEBSITE EXAMPLE ABOVE, THERE'S ALSO A "REQUEST HEADERS" SECTION IF YOU SCROLL DOWN. BUT ACTUALLY, I DON'T THINK THOSE SECTION HEADINGS IN THE CHROME INSPECTION TOOL NECESSARILY CORRESPONDS TO THE 4 TYPES OF HTTP HEADER FIELDS YOU'VE LISTED ABOVE.

FOR EXAMPLE, ACCORDING TO THE "HTTP HEADER FIELDS" LINK ABOVE, ENTITY HEADERS INCLUDE FIELDS LIKE "Content-Length" AND "Content-Type", WHICH IN THIS CASE APPEARS UNDER THE "RESPONSE HEADERS" SECTION. BUT I THINK IF IT WERE A POST REQUEST, THE "REQUEST HEADERS" SECTION MAY INCLUDE SOME ENTITY HEADER FIELDS AS WELL.

SAME GOES FOR GENERAL HEADER - FIELDS SUCH AS "Cache-Control" APPEARS UNDER BOTH THE "REQUEST HEADERS" AND "RESPONSE HEADERS" SECTIONS. SO I THINK THE 4 TYPES OF HEADER FIELDS COULD APPEAR UNDER "REQUEST HEADERS" AND/OR "RESPONSE HEADERS" SECTIONS, BUT THE "GENERAL" SECTION SEEMS TO JUST CONTAIN SOME GENERAL INFO ABOUT THE REQUEST - THEY'RE NOT ACTUALLY HEADER FIELDS.

NOT SURE IF IT'S NECESSARY TO GO INTO ALL THIS DISTINCTION IN THE LECTURE THOUGH! MAYBE IT'S ENOUGH JUST TO MENTION YOU CAN FIND THE HEADER FIELDS LISTED UNDER THE NETWORK TAB WHEN YOU INSPECT? ONE THING THAT MIGHT BE WORTH MENTIONING HERE THOUGH (WHICH COULD TIE IN W/ YOUR LATER SECTION), IS THAT ONE OF THE POSSIBLE REQUEST HEADER FIELDS IS "Authorization", WHICH IS THE FIELD YOU WOULD SPECIFY IF YOU NEED TO PROVIDE AUTHORIZATION FOR AN API REQUEST.
...
- The response message body will depend on two things- the request method and the status code. 
    ...
    [![](https://mdn.mozillademos.org/files/13821/HTTP_Request_Headers2.png){width=80%}](https://developer.mozilla.org/en-US/docs/Web/HTTP/Messages)  
...
[TEXT ABOUT ACS/CENSUS AND TIDYCENSUS PACKAGE]

Tidycensus package (from tidycensus [website](https://walkerke.github.io/tidycensus/index.html))
...
2. Install packages  
    - `library(tidycensus)`, `library(tidyverse)`, and include `options(tigris_use_cache = TRUE)`  
